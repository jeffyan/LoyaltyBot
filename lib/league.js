	...
	var getSoloRank = function(user, id, cb) {
		...
		__self.https.get(__self.lolapi.summoner_league_url + id + '?api_key=' + __self.config.apikey, function(res) {
            ...
            res.on('data', function(d) {
				...
				var json = JSON.parse(buffer);
// console.log(json);		
				var solorank = json[id].tier + ' ' + json[id].rank;
				...
				getSummonerIdByName(user, function(user, id) {
					if (id !== null) {
						getSoloRank(user, id, function(rank) {
							if (rank !== null) {
								__self.irc.emit('message',{message:'> ' + user + '\'s solo rank is ' + rank});
							} else {
								__self.irc.emit('message',{message:'> Can\'t find ' + user + '\'s solo rank...'});
							}
						});
					} else {
						__self.irc.emit('message',{message:'> Nobody knows who ' + user + ' is...'});
					}
				});
				break;
			default:
